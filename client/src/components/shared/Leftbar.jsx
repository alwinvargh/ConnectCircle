    ...
    <div className={`${showLeftbar ? "" : "hidden"} leftbar`}>
      <div className="flex flex-col justify-start items-center">
        <div className="flex flex-col items-start gap-4 w-full p-5">
          <Link
            className="flex items-center gap-2 text-lg font-medium hover:text-purple-700"
            to="/home"
          >
          ...
          {communityLinks && communityLinks.length > 0 ? (
            <div className="w-full">
              <div className="flex items-center justify-between">
                ...
                <Link
                  className="flex relative items-center text-sm font-medium text-purple-700 mr-4"
                  to="/my-communities"
                >
                  See all
                  <p className="absolute -top-2 -right-4 text-white text-xs bg-purple-700 w-4 h-4 rounded-full flex justify-center items-center">
                    {" "}
              ...
              <ul className="w-full mt-3">
                {communityLinks.map((communityLink) => (
                  <li key={communityLink.href}>
                    <Link
                      className="flex items-center hover:text-purple-700 text-gray-600 font-medium gap-2 py-1"
          ...
          {user && user.role === "general" && (
            <div className="md:hidden">
              ...
              <div className="flex justify-center gap-1 items-center">
                <GiTeamIdea />
                <Link to="/communities" className="text-purple-700 font-medium">
                  See all communities
                </Link>
              </div>
            </div>
